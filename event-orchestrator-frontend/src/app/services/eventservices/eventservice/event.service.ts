...
private baseUrl = 'http://apigateway' ;
  ...
  addEvent(formData: FormData, userId: String): Observable<any> {
    return this.http.post(`http://apigateway/Event/addevent/${userId}`, formData);
}
  ...
  exporteventsToExcel(): Observable<Blob> {
    return this.http.get('http://apigateway/Event/export-events', { responseType: 'blob' });
  }
  ...
  getAllCategories():Observable<any>{
    return this.http.get('http://apigateway/Event/eventtypes')
  }
  ...
  getImage(eventId: number): Observable<SafeUrl> {
    const url = `http://apigateway/events/image/${eventId}`; // Use your actual backend URL
